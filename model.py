...
def stable_softmax(x):
    if (config.use_stable_softmax):
        z = x - T.max(x)
        numerator = T.exp(z)
        denominator = T.sum(numerator, dim=1).view(-1, 1)
...
class decoder_attention(nn.Module):
    ...
    def forward(self, s_t, prev_s):
        ...
        if config.intra_decoder is False:
            ...
            at = stable_softmax(et).unsqueeze(1)  # bs, 1, t-1
